namespace EldritchMile.Core
{
    ...
    public interface IProbabilityTarget
    {
        ...
        /// <summary>
        /// 把衰減歸零回 1.0。**由 `DialogueEncounterController.Begin()` 對所有目標呼叫。**
        ///
        /// 【為什麼一定要有這一支】衰減的語意是「**這一個環節之內**越試越難」，
        /// 不是「這個箱子永遠越試越難」。
        ///
        /// 沒有它的時候會發生：玩家打寶箱 A 剩一張牌就離開 → 去別處打完 →
        /// 回來重開環節，**手牌是全新的五張，但 A 的機率還卡在 0%**。
        /// 場上兩個資源一個重置一個沒有，那個箱子從此再也開不了，而且不會有任何錯誤訊息。
        ///
        /// 一個環節 ＝ 一手牌 ＋ 一組全新的衰減。兩者要一起重置。
        /// </summary>
        void ResetDecay();
