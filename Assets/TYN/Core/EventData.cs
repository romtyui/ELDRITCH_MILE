...
namespace EldritchMile.Core
{
    ...
    public class EventData : ScriptableObject
    {
        ...
        [Tooltip("**優先序。數字大的先出。**\n\n" +
                 "同一站只會有一個事件出得來，所以「誰先出」是有意義的 ——\n" +
                 "《暴食之深淵》是這張地圖的開場介紹，理應排在《無人的小船》前面。\n\n" +
                 "【跟 Weight 的分工】\n" +
                 "　· Priority 決定**哪一批**先輪到（大的整批贏過小的）\n" +
                 "　· Weight 決定**同一批之內**誰被抽到\n\n" +
        ...
        [Tooltip("同時有多個事件可觸發時的權重。**只在同一個 Priority 之內比**。\n\n" +
                 "**0 ＝ 關掉這個事件**（`EventLibrary.Pick` 會直接跳過）——\n" +
                 "沒有另一個「啟用」勾勾是刻意的：關掉與「機率很低」本來就是同一件事，\n" +
                 "兩個開關並存的話遲早會出現「明明勾了卻不觸發」。\n\n" +
                 "⚠️ 關掉的理由請寫進 Notes —— 不寫的話下一個人只會看到一個 0，\n" +
