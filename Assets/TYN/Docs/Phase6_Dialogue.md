...
```
攤出來  →  可以打  →  失敗：留著，機率衰減，可以再試
                   →  成功：演一次「霧凝聚又散去」→ **結案**（變暗、不再接受出牌）
        →  三個都成功  →  等動效演完，自動收尾
```

### 為什麼成功要結案

一開始是「成功也留著可以繼續打」（照 C18⑦）。實測時企劃發現這代表
**同一個選項可以重複刷獎勵** —— 五張手牌就能把同一根撬棍刷三根。

先做的補救是「獎勵只發一次」，但那治標：玩家還是會看到「成功」卻沒東西拿。
定案是**成功即結案**，因為對話選項的語意本來就是「問過就問過了」。

| | 結案 | 理由 |
|---|---|---|
| 成功 | ✅ | 已經問出來了，再問一次沒有意義 |
| 失敗 | ❌ | 還能再試 —— 那正是逐次衰減（C18④）存在的理由 |

### 實作上的三個決定

1. **用 `CanvasGroup.blocksRaycasts` 擋掉，不改 `IProbabilityTarget` 介面。**
   「不再接受出牌」等於「不再收到滑鼠事件」，拖曳與點擊兩條路一次擋掉。
   在 Core 的規則引擎裡加「可不可以打」的話，介面與所有實作者都要跟著改，
   而寶箱那邊根本沒有這個概念。
2. **結案放在動效之後。** 先變暗的話，那一下閃光是打在半透明的格子上，
   玩家會覺得「好像有東西閃過去」而不是「這一句成了」。
3. **獎勵只發一次的防線留著。** 結案之後理論上不可能再成功，
   但那個 `HashSet` 很便宜，而且擋得住日後「某些選項可以重複問」這種改動。

### 全部問完會自動收尾

三個選項都成功之後就沒有目標可打了。這時候還把手牌留在畫面上，
玩家會以為自己漏了什麼 —— 所以等動效演完就結束環節。
...
- [ ] 對話：三個選項各自顯示不同機率（邏輯／直覺／批判與創造 —— 2026-08-21 起叫超我／本我／自我）
