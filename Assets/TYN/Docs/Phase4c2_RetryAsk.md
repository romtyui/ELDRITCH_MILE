...
### `AttributeChart` 有一條 `AttrA → AttrC = None`

加這條是為了**測得到 `✕` 顯示**。相剋表原本 `rules` 是空的、`defaultEffectiveness` 是
`Partial`，所以沒有任何組合會算成 `None`，`✕` 那條驗收永遠驗不到。

> C19 明講「`None` 應該少用，只保留給設計上真的要封死的組合」。
> 這條是測試用的，**不是設計定案** —— 屬性正式命名（Q7a）時要一起重看。
...
### 預期數字（測試寶箱是 `AttrC`、手牌 5 張 → 每張衰減 0.2）

| 卡 | 第 1 張 | 第 3 張（衰減 0.6） |
|---|---|---|
| `explore_C_60` | **60** | **36** |
| `explore_B_60` | **30**（`Partial` 0.5×） | **18** |
| `explore_A_60` | **`✕`** | **`✕`** |
