# ELDRITCH_MILE — 系統盤點（做完的／待做的／劇情需要什麼）

> 最後更新：2026-08-15 · 涵蓋 `Assets/TYN/`（探索、地圖、流程、對話、道具）
>
> **這份文件在補一個缺口**：工程文件（`RoadmapNext.md`、各 `Phase*.md`）只談程式，
> 劇情文件（`克蘇魯劇情大綱.docx`）只談內容，中間沒有橋。
> 結果是「劇情裡寫了的東西需要哪些系統」這件事沒有任何地方記錄。本文負責這一段。
>
> 讀這份文件之前建議先看 [HANDOFF.md](HANDOFF.md)（慣例與踩坑）。
> 「接下來做什麼」的細節仍以 [RoadmapNext.md](RoadmapNext.md) 為準，本文只給全景。

---

## 0. 一頁總覽

| 系統 | 狀態 | 歸屬 | 劇情裡誰依賴它 |
|---|---|---|---|
| 流程總管（Stage／地圖覆蓋層） | ✅ 可用 | TYN | 全部 |
| 地圖生成與節點 | ✅ 可用 | TYN | 「關聯場景：港口、漁村」 |
| 主選單 | ✅ 可用 | TYN | — |
| 地圖動效（節點 tooltip／棋子滑行） | 🔶 程式完成，**tooltip 面板待建** | TYN | — |
| 探索房間（隨機填充、互動物件） | ✅ 可用 | TYN | 全部 |
| **打牌判定（C17/C18/C19）** | ✅ **已驗收** | TYN | 探索的核心迴圈 |
| 屬性與相剋 | ✅ 已定案 | TYN | 尚未接上世界觀 |
| 道具資料層 | 🔶 資料層完成，**無 UI** | TYN | 漁夫的漁獲、商店 |
| 道具數量 | ✅ `ItemStack`（可容納 per-instance 狀態） | TYN | 漁獲、商店 |
| 對話系統 | 🔶 **已實作，待驗收**（選項＝判定目標，可用機率卡打） | TYN | **五名角色全部** |
| 商店 | 🔶 **已重寫，待驗收**（八格貨架、有價格、依區域換貨） | TYN | 商人（冷原人） |
| 事件效果（陷阱／逆轉／複製卡牌） | ❌ 未做 | TYN | **畫家/卡羅麗** |
| 神牌 | 🔶 事件端已實作（挑一張牌）；真正的神牌仍待戰鬥端 | TYN + Romtyui | **小說家/克拉夫特** |
| HP／SAN 的探索端讀寫 | ✅ **已接**（`PlayerVitals` 轉接頭，零修改隊友檔案） | TYN | **漁夫、貴婦** |
| 侵蝕度 | 🔶 **資料層完成**（每尊神一條），尚未有 UI | TYN | 角色倒戈條件、**深淵線事件的觸發門檻** |
| 事件系統 | 🔶 **資料層完成，還沒有 Stage 播得出來** | TYN | 深淵線、常駐事件、新手教學 |
| 戰鬥 | 由 Romtyui 負責 | Romtyui | 全部 |
| 戰鬥接入探索流程 | 🔶 **下週三／四對接** | TYN + Romtyui | — |

---

## 1. 已完成（可用、且大多已在 Play Mode 驗收）

### 1.1 流程骨架

單一 `EventScene`，Build Settings 只有一個場景。

| 元件 | 職責 |
|---|---|
| `GameFlowManager` | 唯一的流程總管。Stage 切換與地圖開合都必須經過它。也持有 `RunContext` 與 `MetaProgressData` |
| `StageHost` / `StageController` | Stage 的生成與生命週期（`OnStageEnter` → `OnStageReady` → `OnStageExit`） |
| `UIDirector` / `UIPanel` | UI 開關總管。`Panel` 走狀態機、`Dialog` 走堆疊、`Widget` 不管 |
| `MapOverlayController` / `MapView` | 地圖覆蓋層（不是獨立場景，是覆蓋層） |
| `ScreenFader` / `FadePanel` / `PanelToggle` | 轉場與面板顯示 |
| `PopupService` / `DialogueBoxUI` | 全專案共用的訊息與對話框（含打字機、排隊、公版格式） |

> 文件：[Phase1_EditorSetup.md](Phase1_EditorSetup.md)、[Phase2_MenuStage.md](Phase2_MenuStage.md)、[Phase3_MapOverlay.md](Phase3_MapOverlay.md)

### 1.2 探索房間

`RoomLibrary` → `RoomContentData` → `SpawnSlot` 隨機填充，用 run seed + nodeId 保證可重現。

互動物件收斂成 `InteractableBase`：
- `ChestInteractable`（`Direct` / `RequiresKey` / `RequiresCheck` 三態）
- `InspectableInteractable`

C13 的迴圈：房間清空**不會**自動離開，而是問「要探索其他的東西嗎？」。

> 文件：[Phase4a_ExploreStage.md](Phase4a_ExploreStage.md)

### 1.3 打牌判定 — 本階段的主要成果

四批做完，全部在 Play Mode 驗收過：

| 批次 | 內容 | 文件 |
|---|---|---|
| 一 | 拖曳／兩段式出牌、hover 全選項預覽、即時判定、逐次衰減、對話框內對象大圖 | [Phase4c_CardPlay.md](Phase4c_CardPlay.md) |
| 二 | C12 回合感（次數提示，**取代**了原本的確認視窗） | [Phase4c2_RetryAsk.md](Phase4c2_RetryAsk.md) |
| 三 | 徹底失敗結案、房間清空修復、付代價重試（道具） | [Phase4c3_RetryCost.md](Phase4c3_RetryCost.md) |
| 四 | Q7a 屬性定案、相剋表、手牌變暗 | [Phase4c4_Attributes.md](Phase4c4_Attributes.md) |

**規則全部在 `DialogueEncounterController`（Core）**，UI 層只畫畫面與轉交動作，不做任何判斷。

屬性（Q7a 定名 → Q7b 改名，都已定案）：

| ID | 顯示名 | 顏色 | 值 |
|---|---|---|---|
| `None` | 無 | 黑白 | 0 |
| `Id` | 本我 | 紅 | 1 |
| `Superego` | 超我 | 藍 | 2 |
| `Ego` | 自我 | 綠 | 3 |

相剋：**本我 ↔ 超我互斥（0×）**，自我居中（0.5×），同屬性相符（1.0×）。
這不是元素環 —— 目標的屬性代表「這個難題吃哪一種心智」。

#### 2026-08-21 改名：直覺／邏輯／批判與創造 → 本我／超我／自我

用佛洛伊德的心靈三分取代自訂分類。**數值完全沒動，所以卡牌、物件、相剋表、牌框一個都不用改。**

改名的理由：
- 「批判與創造」是三個裡唯一的複合詞，讀起來像佔位，而且**文案沒辦法把它寫成場景**
- 佛洛伊德那套是**現成的三元組**，玩家腦子裡已經有這張表，不用重新學
- 與 SAN、心智碎裂的題材是同一件事的兩個面（年代也對）

**為什麼是「本我 ↔ 超我對立、自我調停」而不是別的對法**：
現有相剋表本來就是「1 和 2 對立、3 是橋」，而佛洛伊德那套剛好同構 ——
慾望與戒律對立、自我夾在中間斡旋。所以是**零成本**。
對成「自我 ↔ 超我對立」的話功能一樣，但要改相剋表 ＋ 對調 12 張卡的牌框，
而且會跟玩家既有認知打架（大家都知道調停的是自我）。

紅藍留給真正對立的那一對 —— **紅藍是辨識度最高的對立組合**，
而玩家判斷「這張牌對這個目標有沒有用」靠的是顏色，不是文字（屬性名玩家看不到）。

> 顯示名現在是 `AttributeChartData` 上的欄位（`idName` / `superegoName` / `egoName`），
> 之後要再改名改資產就好，不用動程式。

> **關於「綠比較強」——我先前judgment錯了，這裡更正。**
>
> 我算過綠的期望值 0.67 > 紅藍 0.50，把它記成平衡債。**那個算法用錯了前提**：
> 它假設「隨機一張牌對上隨機一個目標」，但實際的決策是
> 「**面對這個目標，我手上哪張最好**」。在那個框架下：
>
> | 目標是紅 | 紅牌 1.0 | 綠牌 0.5 | 藍牌 0 |
> |---|---|---|---|
>
> **綠永遠不是最佳解，只是「最好的可用解」** —— 它是保險牌，紅藍是對上了更強。
> 這是健康的取捨，不是失衡。
>
> 而且探索牌組**不開放玩家自主編成**，所以「玩家會把牌組堆成綠」這個風險根本不存在。
> 剩下的只有牌組配比，那是可調的內容參數，不是機制債。
>
> （企劃補充：100% 的牌若非同屬，第一輪打出去也只是 50/50。）

### 1.4 道具資料層（剛完成）

| 元件 | 職責 |
|---|---|
| `ItemData`（SO） | `id` / `displayName` / `icon` / `description` |
| `ItemDatabase`（SO） | id → ItemData 查表 |
| `GameFlowManager.ItemName(id)` | 靜態便利方法 |

**`RunContext.inventory` 是 `List<ItemStack>`** —— id 是真相，`ItemData` 只負責「長什麼樣」。
數量與日後的 per-instance 狀態都掛在 `ItemStack` 上，見 §4。

---

### 1.5 地圖 UI 動效（2026-08-16）

| 項目 | 狀態 |
|---|---|
| 棋子移動：彈跳 → 磨砂石桌上的滑行 | ✅ 有預設值，**不用設定就生效** |
| 節點 hover：縮放 → tooltip | 🔶 程式完成，**面板要在編輯器裡建**（10 分鐘） |
| 游標變化 | ❌ 等美術素材。機制現成，插入點已確認 |

⚠️ **tooltip 面板建起來之前，地圖 hover 完全沒有回饋。**
趕時間可把 `MapNodeUI.Scale Hover` 設 `1.1` 暫時頂著（預設 `1` ＝ 關閉）。

操作與調參見 [MapPolish.md](MapPolish.md)。

---

### 1.6 資料庫、商店與對話氣泡（2026-08-16）

| 元件 | 職責 |
|---|---|
| `LootTable`（SO）＋`LootService` | **誰在什麼場合會出現什麼**。表 → 池 → 條目，權重隨機 |
| `CharacterData` / `CharacterDatabase`（SO） | id → 角色（名字／立繪／寒暄／閒聊） |
| `ShopPanelUI` / `ShopSlotUI` | 八格貨架，圖示＋價格＋已售出 |
| `SpeechBubbleUI` / `CharacterHitbox` | 常駐對話氣泡（bark）＋ 背景角色的隱形點擊區 |

三件事定了下來，之後不要再各自發明第二套：

1. **權重寫在表上，不寫在物品上** —— 物品的 `tags` 只說「它是什麼」
2. **難度／區域＝換一張表**，`ChestInteractable` 與商店共用同一套抽取
3. **bark 與對話樹分開** —— 氣泡是氣氛，對話框是內容

`ItemData` 因此多了 `price` 與 `tags`；`RunContext` 多了 `money`。
**錢的一切（名稱、來源、經濟）都還沒設計**，`startingMoney` 是佔位值。

商店的設定由企劃補齊了（大綱 docx 裡沒有這一節）：**移動商店、店主是冷原人商人**，
21 句台詞照原文存在 `Character_merchant.asset`。

商品三類，用詞照坎貝爾在教學裡說的 —— **消耗品、武器...以及收藏品**。
商人到處跑，但貨表**依區域替換**：漁村那張食物格多一倍、且以漁獲為主，
兩張表共用同一組武器池與收藏品池。

**收藏品是大綱〈收藏品〉表的真品 8 件**（老舊釣竿、貪婪的大口、人魚肉、螺湮御守…），
含敘述與效果，並帶品質標籤 `Common`/`Uncommon`/`Rare`，抽取配比 50/33/17。

> **品質與掉落率是兩件事**：品質屬於物品（人魚肉永遠是稀有），
> 掉落率屬於表（第一區 5%、最終區 30%）。這跟「不要把 tier 寫在物品上」不衝突，
> Slay the Spire 也是這樣切的。

⚠️ 這些收藏品的效果**幾乎都在戰鬥中生效**，所以是跨隊需求（見 §2.1）。

細節、業界依據與已驗證項目見 [Phase7_ShopAndData.md](Phase7_ShopAndData.md)；
大綱完整版讀後的系統缺口見 §3.0。

---

## 2. 待做，依「被什麼擋住」分類

### 2.1 🟢 原本卡跨隊的三件事 —— 2026-08-16 已談定

| # | 需求 | 結論 |
|---|---|---|
| A | 戰鬥結束事件 / 戰鬥接入流程 | **下週三或四對接**。隊友會盡量把戰鬥包成 prefab |
| B | run 開始就初始化 HP／SAN | ✅ **已實作**（`PlayerVitals`）。零修改隊友的檔案，只呼叫 |
| C | 侵蝕度歸誰管 | **歸我方** |

#### B 怎麼做的：轉接頭，不是搬家

`Core/PlayerVitals.cs` 是探索端存取 HP／SAN 的唯一入口。

**資料沒有搬過來** —— 它還是存在隊友的 `RunStateManager`。我方只呼叫，
**沒有改到 `Assets/Romtyui/` 的任何一個字**。搬過來的是「誰負責在 run 開始時初始化」這個責任。

> 為什麼不各存一份：只要有一條路徑忘了同步，玩家就會看到
> 「探索扣了血，進戰鬥又滿血」—— 而且不會報錯，只會慢慢對不起來。

**⚠️ SAN 在隊友的程式裡叫 `Energy`。**
`RunStateManager` 沒有任何 sanity 欄位，它的 `savedCurrentEnergy` / `savedMaxEnergy` 就是 SAN ——
證據是他自己的 log 印 `SAN {savedCurrentEnergy}/{savedMaxEnergy}`，
而 `BattleManager` 有一行註解寫「因為你要求 SAN 值不重製」（一般 energy 每回合會重置，SAN 不會）。
**這個對應只寫在 `PlayerVitals` 一個地方**，其他程式一律用 `San`。

**⚠️ 初始化會讓 `hasSavedRunState` 變 true**，那是隊友 `ApplyToBattle()` 的開關 ——
往後戰鬥開始會套用我們設的血條，而不是戰鬥自己的預設值。
**那正是「run 開始就初始化」的用意**，但數值要跟戰鬥組對一次。
`maxHp <= 0` 會被擋下並警告（寫下去的話玩家一進戰鬥就是 0 血）。

**要設定的**：`GameFlowManager.Starting Max Hp` / `Starting Max San`。
**預設是 0 ＝ 不初始化**，維持舊行為，所以不會無聲改變現況。

順帶可提：`RunStateManager.cs` 的中文註解疑似不是存成 UTF-8（讀出來是亂碼）。

**已經解鎖的**：

- `RetryPolicy.CostsHealth` / `CostsSanity` **接上了** ——
  扣不動就會說「付不起」並結案，而且**扣不死人**（扣到 0 以下一律視為付不起）。
  提示文字也會顯示現在的 HP／SAN，讓玩家判斷划不划算。
- C 一定案，深淵線事件的觸發條件（「侵蝕度 50% 以上」）才有東西可以判斷。
  **它是每尊神各一條，不是單一全域值**

### 2.2 🟡 設計未定（等企劃決定，不是工程問題）

| 項目 | 卡在哪 |
|---|---|
| 一般物件 vs 特殊物件的分類機制 | 「特殊效果」目前不存在於資料模型。物件數量還太少，硬做分類會變空想 |
| 大類型的固有倍率清單 | 暫用 Tier1~4 + TacticalDeath（×1/2/3/5/20），等物件多了重整 |
| ~~屬性接上世界觀~~ | **2026-08-21 結案** —— 改用本我／超我／自我，見 §1.3。心靈三分與 SAN、心智碎裂本來就是同一件事 |
| ~~綠屬性的平衡~~ | **2026-08-16 結案，不是問題**。見 §1.3 下方的說明 |

### 2.3 🟢 純工程待做（沒有阻塞，隨時可以開）

| 項目 | 份量 | 備註 |
|---|---|---|
| **地圖 tooltip 面板** | 小 | 10 分鐘。做完 hover 才有回饋，見 [MapPolish.md](MapPolish.md) |
| 背包 UI | 中 | 玩家目前看不到自己身上有什麼 |
| Phase 4d 收尾驗證 | 小 | 鑰匙寶箱、兩段式離開（C14）。⚠️ `chest_RequiresKey` 權重目前是 0（測試腳手架） |
| Phase 5 我方準備 | 中 | 從 Romtyui 的 Scene 打包 prefab、包 `BattleStageController`、檢查 Camera/EventSystem/AudioListener 重複 |
| Phase 6 對話系統 | 大 | 見 §3，**劇情最依賴這個** |
| Phase 6 商店（C15） | 中 | 需要道具數量與價格 |
| C18① 主要目標選定 | 小 | 併入 Phase 6。手牌變暗已經給了它明確用途 |
| C18③ 選項內文即時更新 | 小 | 需要選項有文字元件，Phase 6 一起 |
| Phase 7 神牌 | 小 | 見 §3 |
| repo 加 `.editorconfig` | 極小 | [RoadmapNext](RoadmapNext.md) §1.7，已評估後刻意延後 |

---

## 3. 劇情大綱 → 系統需求對照

> 依據 `克蘇魯劇情大綱`（2026-08-16 拿到完整的 markdown 版，889 行）。
> **這一節是本文件的重點** —— 它是唯一記錄「劇情裡寫的東西需要哪些系統」的地方。
>
> ⚠️ 之前只讀得到 docx，而那個檔案**只有〈角色〉一節**。
> markdown 版多出：主線教學、深淵線、事件、世界觀＋神明、地區＆怪物、商店、收藏品、探索。

### 3.0 完整版讀後：新增的系統缺口

以下全部是**大綱寫得很具體、但專案裡完全不存在**的東西。按「擋不擋得住現在的工作」排：

| 缺口 | 大綱怎麼寫的 | 影響 |
|---|---|---|
| **侵蝕度** | 深淵線事件標著「（【深淵】的侵蝕度 +5%）」「+10%」，且《海市蜃樓》的觸發條件是「侵蝕度 50% 以上」 | 這就是我一直記成「世界污染進度」的東西。**它是每尊神各一條**，不是單一全域值。事件觸發條件依賴它 |
| **收藏品效果** | 8 件全部寫好了效果（抽【深淵】卡、HP 恢復 +10%、反擊額外發動…） | 幾乎全是**戰鬥中生效**，所以是跨隊需求 |
| **米可（魔法書）** | 回收用過的牌、牌用完會重發、會吃武器牌、可查看牌庫與情報碎片 | 是**牌庫管理器＋圖鑑**。探索的補牌規則已經照這個做了，但「查看」介面沒有 |
| **隊伍（協助者）** | 「四個人的話就是上限了」＝主角＋3 名協助者 | 商人的彩蛋條件、協助者卡牌、協助者互相關係都靠它 |
| **神牌規則** | 神牌賦予武器牌力量；**一張武器牌只能被一位神明賦予**；不同神明之間有相性；使用加重 SAN 負擔；被賦予的牌不會消失 | 這是一整套裝備／詞條系統，比「特殊事件給一張牌」複雜得多 |
| **事件系統** | 「每當前進到下一張地圖，滿足條件就有概率觸發事件」「從**未觸發過**的事件中隨機」 | 需要「已觸發過」的紀錄。條件包含持有數量、時間、擊殺紀錄 |

### 3.0.0 一個發現：角色池、事件、戰利品是**同一個形狀**

（2026-08-18，回答「對話要從區域角色池隨機一個，符合條件時特定角色加入」）

這三件事拆開看是三個需求，拆到底其實是同一句話：

> **一份候選清單 → 濾掉條件不成立的 → 依權重隨機挑一個**

| | 候選 | 條件 | 不重複 |
|---|---|---|---|
| 戰利品（**已完成**） | 道具 | —— | 池內 distinct |
| **角色池**（**已完成**） | 角色 | 「坎貝爾在隊伍」「侵蝕度 ≥ 50%」 | 不需要 |
| **事件**（**已完成**） | 事件 | 持有物、時間、擊殺紀錄 | **未觸發過** |

`LootTable` 已經把「候選＋權重＋隨機」做完了，
`CharacterData.conditionalChatter` 已經把「條件成立才加進池子」做過一次。
**缺的只有中間那一層：條件。**

#### 所以建議：先做「條件」，不要做三套

**① `RunContext` 加一個 flags 清單**（`List<string>`）。
這一個欄位同時解決：已觸發過哪些事件、坎貝爾在不在隊伍、是不是第一次進商店。

> 「未觸發過」不需要特別做 —— 它就是「`event_xxx_done` 這個 flag 不成立」。

**② 一個小小的條件求值器**，先支援三種就夠：
持有某道具 ≥ N ／ 侵蝕度 ≥ N% ／ 某個 flag 成立。

**③ 角色池與事件池都掛上去**，形狀跟 `LootTable` 一樣：

```
CharacterPool（漁村）
├─ TagQuery [漁村]      權重 100   無條件      ← 「該區域的角色池」
└─ Item     時藏         權重 40    深淵≥50%   ← 「符合條件時特定角色加入」
```

角色本來就有 `tags`，所以「該區域的角色池」＝一次標籤查詢，跟道具用的是同一招。

#### ⚠️ 因此角色池要**跟事件系統一起做**，不要先做

單獨先做角色池的話，會為了它做一套只給角色用的條件判斷；
等事件系統來的時候再做第二套，兩套的條件語法還會不一樣。
先把條件那一層做出來，兩邊都只是薄薄一層設定。

---

### 3.0.0b 事件系統與侵蝕度 —— 條件層與資料層已完成（2026-08-18）

**做完的（已離線驗證）**

| 元件 | 職責 |
|---|---|
| `RunContext.flags` | 這場 run 發生過什麼。一個字串清單解決三件事 |
| `RunContext.corruption` | **每尊神各一條**（0–100）。目前只有 `abyss` 確定 |
| `GameCondition` | 條件。**事件池與角色池共用同一個型別** |
| `EventData` ＋ `EventEffect` | 一個事件：條件、機率、內文、選項、效果 |
| `EventLibrary` | 候選 → 濾條件 → 依權重挑一個 |

大綱裡實際出現的六種條件全部驗過：深淵 ≥ 50%、神牌 < 3、武器牌 ≥ 20、身上有糧食、
已玩 ≥ 800 秒、首次踏入（旗標）。另外驗了門檻擋得住、只觸發一次、
事件自己的 30% 機率 300 次中 90 次。

**兩層機率是刻意的**：`EventLibrary.globalChance`（這一站要不要有事件）
與 `EventData.chance`（就算選到我也只有 30% 會發生）。
壓成一層就表達不出《螺湮的祝福》的「打倒祭司後 30% 概率」。

**教學不是另一套機制**：`once = true` ＋ `chance = 1` ＋ 條件是「旗標還沒立」。

#### ✅ 播得出來了（2026-08-18 下午，已實機驗收）

原本缺的兩塊都補上了：

1. **`EventStageController`** —— 內文一句一句播 → 選項（`PlainChoice`，不用打牌）
   → 結果 → 套用效果。**有自己的結束鍵，且要等文字播完才出現**
   （沿用對話框的全幅推進鈕會讓玩家隨手一點就跳過整個事件）
2. **`GameFlowManager.EnterNodeRoutine` 的觸發點** —— 進節點時、Stage 載入**之前**
   檢查一次，有事件就先播；播完由 `StageCompleteRoutine` 接回原本那個節點。
   **前置，不覆蓋** —— 玩家不會因為運氣好觸發事件反而少玩到一間房

#### 四種效果還沒有系統可以接

`DestroyWeaponCard`（喂米可）、`TeleportRandomNode`（門扉）、
`StartBattle`（貪吃鬼選項 B）、`GrantGodCard`（損壞的祭壇）。

它們**仍然列在 enum 裡**，因為文案現在就要寫得出來；執行時會在 Console 說明並跳過，
**不會靜靜地什麼都沒發生**。等對應系統做好補上那一個 case 即可。

#### 已建的內容

大綱〈事件〉章的**八個事件全部建好並登記**（2026-08-18）：
無人的小船、海市蜃樓、暴食之深淵、損壞的祭壇、喂米可吃飯、門扉、好餓好餓的貪吃鬼、螺湮的祝福。
內文與效果照大綱原文，每個資產的 `notes` 都寫明了它卡在哪。

過程中補了一種效果 **`ConsumeItemByTag`** —— 《貪吃鬼》寫的是「消耗多少**糧食**」，
不是某一條魚；文案不會（也不該）指定要扣哪一種，那是玩家背包當下有什麼決定的。
底層是 `RunContext.ConsumeByTag()`，一樣是全有或全無。

⚠️ `EventLibrary.globalChance` 目前是 **1**（測試用），正式配內容前要調低。
⚠️ `Event_gluttony_abyss` 權重是 **100** —— 大綱的條件是「首次踏入漁村」，
但區域系統不存在，所以用高權重逼近「盡量第一個出現」。區域做好後改條件、權重調回 1。

---

### 3.0.0c 角色池 —— 條件層的第三個用戶（2026-08-18）

回答的是「**對話從該區域的角色池隨機一個，符合條件時特定角色也加入**」。

`CharacterPool` 是一張候選名單，三種條目：

| 條目 | 意思 |
|---|---|
| `Character` | 指名某個角色 id |
| `TagQuery` | 任何帶這些標籤的角色 —— **加新角色不用回頭改池子** |
| `Pool` | 轉去另一個池子，共用的名單只寫一次 |

每一條有**權重**與**條件**（就是事件用的那個 `GameCondition`，語法一模一樣）。

#### 為什麼不是 `LootTable` 那種三層（表 → 池 → 條目）

戰利品要「抽 6 次、不重複、再從另一個池抽 2 次」，所以需要池。
角色池永遠只挑**一個**人 —— 一段對話只有一個說話者。
硬套三層只會讓每張角色池資產都多一個永遠只有一個元素的 `pools` 陣列。

需要「一次挑兩個人」的那天（雙人對話），做法是呼叫兩次並排除第一個，不是加一層池。

#### 掛在哪

`DialogueStageController.speakerPool`。填了就蓋過手填的 `speakerCharacterId`，
**挑不到人時退回手填的那個** —— 所以池子壞掉不會讓節點變成沒人說話。

**亂數綁節點**（`runSeed ^ nodeId ^ 鹽`），跟商店的「賣什麼」同一個道理：
玩家離開再進來看到的必須是同一個人，否則等於可以重骰到自己想見的角色。
鹽是為了跟事件的抽獎錯開 —— 兩者在同一個節點抽，直接共用種子的話兩條抽獎會完全相關。

#### 已建的內容

`Chars_Dialogue_Village`，已掛上 `Stage_Dialogue`：

```
[指名 campbell]     權重 100   無條件
[TagQuery 漁村]     權重  40   【深淵】侵蝕度 ≥ 50
```

侵蝕度 0 時只有第一條合格 → **行為與掛上去之前完全相同**（坎貝爾）。
侵蝕度 60 時實測 400 次：坎貝爾 283／時藏 117（期望 286／114）。同種子重複挑 50 次穩定不變。

#### ⚠️ 素材缺口：池子挑到的人不保證素材齊全

| 角色 | 立繪 | 招呼 | 閒聊 | 成功 | 失敗 | 結語 |
|---|---|---|---|---|---|---|
| 坎貝爾 | ✅ | 3 | 2 | 3 | 3 | 3 |
| 時藏 | ❌ | 3 | 4 | **0** | **0** | **0** |
| 商人 | ❌ | 3 | 13 | — | — | — |

手填 id 的年代是人工確認過的，現在換人是隨機的 —— 缺什麼只會靜靜地不顯示。
所以 `ResolveSpeaker()` 會在挑到「沒立繪」或「沒有成敗台詞」的人時**發警告**。
時藏要真的能在對話節點上場，缺的是立繪與成功／失敗／結語三組台詞。

---

### 3.0.1 探索地點的分類 —— 這個可以直接用

大綱把可探索地點分成**小屋／中屋／大屋**，而且列出了每種裡面有什麼：

| 漁村 | 可探索 |
|---|---|
| 小屋・漁人小屋 | 爐子（**必定出食物**）、箱子（武器食物隨機） |
| 中屋・民宅 | 壁櫥、櫃子、箱子 |
| 大屋・集會所 | 壁櫥 ×2、箱子 ×2、桌子（**概率獲得食物**） |

這剛好對上現有的 `RoomLibrary`（哪種房間）＋ `RoomContentData`（房間裡放什麼）
＋ `LootTable`（每個容器開出什麼）。三層都已經有了。

#### ✅ 2026-08-18 已填完，而且**沒有改到任何程式**

| 房間 | 槽 | 內容 | 抽中機率 |
|---|---|---|---|
| `Room_Village_Small`（漁人小屋） | 2 | 爐子 ＋ 箱子 | 50 |
| `Room_Village_Medium`（民宅） | 3 | 壁櫥 ＋ 櫃子 ＋ 箱子 | 35 |
| `Room_Village_Large`（集會所） | 5 | 壁櫥×2 ＋ 箱子×2 ＋ 桌子 | 15 |

**為什麼不用改程式**：`RoomLibrary.Pick()` 在節點的 `contentId` 留空時，
本來就會「從同 kind 的房間依權重抽」。所以三種屋子登記進去、權重填好，就會隨機出現 ——
`contentId` 留給日後手工指定的特殊房間用。

五種容器（`village_Stove` / `_Box` / `_Closet` / `_Cabinet` / `_Table`）
是以既有的 `chest_RequiresCheck` 複製出來的，因此美術、碰撞、UI 掛法全部沿用。
**全部是 `RequiresCheck`** —— 大綱寫「玩家可以在這些場所内**透過卡牌**來獲取隨機的資源」。
屬性刻意分散（爐子本我／箱子超我／壁櫥自我／櫃子超我／桌子本我），
讓紅藍綠在同一間房裡都有用武之地。

大綱明寫的兩條規則直接對應到表：
「爐子**必定**出食物」＝`chance 1` 的池子；
「桌子**概率**獲得食物」＝`chance 0.5`（實測 200 次中 102 次）。
食物一律轉去 `Loot_Sub_Food_Village`（漁獲 80／一般糧食 20），
與商店共用同一份配比，不重寫第二次。

⚠️ **槽的位置是沿用舊測試房的座標，等美術對位。** 大屋的第五個槽是我隨便補的位置。
⚠️ 舊的 `Room_Village_01` 權重已設為 0（不再被抽到），確認新的沒問題後可以封存。

### 3.0.2 區域清單（合併地區章節與商人的閒聊）

城市（中立，各區怪物都會來）／教堂（debuff）／港口／漁村（糧食多）／
農舍・牧場（偽裝）／劇場（持續傷害）／森林（多體）／礦山（金幣多）

`RunNodeData` **還沒有 region 欄位**，商店目前借用 `contentId` 比對。

### 3.1 角色的三重身分

劇情設定：角色是「千面的奇術師」收集情報時混入的**情報碎片**，被設置為**對應侵略者的錨點**，
散落世界各處。他們**可以接觸與對話，但無法干涉玩家的行為，也不能直接參戰**；
不過被賦予了**作為卡牌在戰鬥中提供幫助**的能力。

這代表每個角色同時是三種東西，而且**分屬不同的系統與不同的負責方**：

| 身分 | 需要什麼 | 歸屬 | 現況 |
|---|---|---|---|
| **可對話的 NPC** | 對話系統（`DialogueData` + `DialogueStageController` + 多選項） | **TYN** | ❌ 未做（Phase 6） |
| **戰鬥中的卡牌** | 卡牌效果系統 | Romtyui | 由對方負責 |
| **可能倒戈的敵人** | 敵人資料 + **觸發條件** | Romtyui + **觸發條件見 §3.3** | ❌ |

> 對話框已經支援立繪（`DialogueBoxUI.portraitRoot` / `EncounterTargetView`），
> 所以角色的「人類／非人」兩種外貌在技術上已經有地方擺。

### 3.2 各角色的能力 → 對應的系統缺口

| 角色 | 劇情寫的能力 | 需要的系統 | 歸屬 | 現況 |
|---|---|---|---|---|
| **魔術師/坎貝爾**<br>（奈亞拉托提普） | 抽取兩張手牌 | 卡牌效果 | Romtyui | — |
| **小說家/克拉夫特** | 杜撰故事 — 抽取 1 張**神牌** | **神牌系統**（Phase 7） | TYN 給牌 + Romtyui 定效果 | ❌ |
| **畫家/卡羅麗**<br>（無定形的色彩） | **觸發事件**可複製一張現有的卡牌 | **事件效果系統** | **TYN** | ❌ **完全不存在** |
| **漁夫/時藏**<br>（克蘇魯／深潛者） | 隨機獲取漁獲<br>（使用後 **+HP −SAN**，**只能在戰鬥外使用**） | **背包 + 可使用道具 + HP／SAN 橋** | **TYN** + 跨隊 B | 🔶 資料層剛做，UI 與使用動作未做 |
| **貴婦/伊麗沙白** | 吸血、**消耗 HP 獲得收益** | HP 橋 | 跨隊 B | ❌ |

#### 三個從劇情反推出來、工程文件裡原本沒有的需求

**① 漁夫的漁獲＝可主動使用的消耗品。**
「只能在戰鬥外使用」明確要求探索階段能：看到道具 → 點它 → 使用 → 改變 HP／SAN。
這把背包從「Phase 6 商店才需要」提前成「設計上已經存在」。
它同時要**跨隊需求 B**（HP／SAN 初始化）才能真的動。

**② 畫家的能力是「觸發事件」，那是 TYN 的範圍。**
「觸發事件可複製一張現有的卡牌」不是戰鬥中的卡牌效果，是**探索／事件階段**的效果。
但 TYN 目前沒有任何「事件效果」的表達方式 —— `ChestInteractable` 只有
`openMode` / `attribute` / `lootItems` / `grantedItemIds`。
這與 §2.2 的「一般物件 vs 特殊物件」是同一個缺口，**陷阱與逆轉也會落在這裡**。

**③ HP 與 SAN 是互相交換的關係。**
漁獲是 `+HP −SAN`、貴婦是「消耗 HP 獲得收益」。所以這兩個數值不是各自獨立的血條，
而是一組可以互相轉換的資源 —— 這會直接影響重試代價要扣哪一個、扣多少。

### 3.3 世界污染進度 — 一個不存在的變數

> 劇情大綱：「依照世界被污染的進度。他們有概率會提前遭到侵略者污染，
> 成為強力的眷屬與主角敵對。」

這是決定**盟友何時倒戈**的全域進度值。目前：

- `RunContext` 沒有（只有地圖、探索牌組、背包）
- `RunStateManager`（Romtyui）沒有（只有 HP／能量／戰鬥牌組）

**需要企劃決定兩件事**：

1. 它是不是就是 **SAN**？（理智下降＝世界看起來更污染，在克蘇魯題材裡說得通）
   還是獨立的第三個值？
2. 若獨立，**歸誰管**？影響戰鬥（敵人組成）也影響探索（誰還能對話），兩邊都要讀

> 這是跨隊需求 **C**，建議跟 A、B 一起提。

### 3.4 場景需求

漁夫的設定寫了「**關聯場景：港口、漁村**」。目前 `RoomContentData` 只有
`RoomContent_Village` 一份，`RoomLibrary` 也只對應到兩個 prefab。

角色若要綁定特定場景出現，需要：節點類型 → 場景類型 → 內容表的對應鏈。
骨架（`RoomLibrary.Pick(node, rng)`）已經在了，缺的是內容。

---

## 4. 道具數量：`ItemStack`（2026-08-15 定案並實作）

```csharp
[Serializable]
public class ItemStack { public string id; public int count = 1; }

// RunContext
public List<ItemStack> inventory;
public int  CountOf(string id);                       // 跨疊加總
public void AddItem(string id, int count = 1);        // 同 id 併疊
public bool ConsumeItem(string id, int count = 1);    // 全有或全無
```

### 為什麼是這個結構（查證過業界做法後的結論）

業界的分界線只有一條：**這個道具有沒有「每一份各自不同」的狀態。**

- **可堆疊**（三根撬棍完全一樣）→ 用 `count`
- **不可堆疊**（耐久度、詞綴、隨機屬性）→ 每份都要是獨立實例

通用寫法是 `IsStackable()` + `Count()`，不可堆疊的 count 恆為 1。
另外，「重複清單 + 顯示時才 group」也是被認可的做法（在 GUI 層堆疊而非 model 層），
**不是偷懶** —— 多數 Unity 教學展示 `{item, quantity}` 是因為它們在做格子背包，
槽位本身就是資料模型，那個理由不適用於本專案。

### 決定性的因素：漁獲

劇情大綱裡漁夫的能力是「**隨機**獲取漁獲」。
若每條魚的 `+HP／−SAN` 數值不同，漁獲就不是可互換的東西 ——
`List<string>` 與單純的 count 都裝不下它。

遊戲是肉鴿，這類「同名但數值不同」的東西只會變多。
**`ItemStack` 能表達「三根一樣的撬棍」，反過來不行**，所以選它。

換的時機也對：`RunContext` **目前完全沒有持久化**（只有 `MetaProgressData` 用 PlayerPrefs），
所以沒有舊存檔要顧；等商店與漁獲都接上去之後呼叫點會多很多。

### 幾條實作上的約束

**① `ConsumeItem` 必須全有或全無。** 需要 3 個但只有 2 個時，不可以扣掉那 2 個再回報失敗
—— 那是這類 API 最典型的 bug，而且因為資源真的少了，重試也修不回來。
程式是先 `CountOf` 確認付得起才動手扣。

**② 允許同一個 id 出現在多疊裡。** 這是刻意的，per-instance 狀態就靠它表達。
所以 `CountOf` 與 `ConsumeItem` 都跨疊處理，不可以假設「一個 id 只有一疊」。

**③ 加 per-instance 狀態時，`AddItem` 的合併規則要跟著改。**
目前是「同 id 就併」（道具還都可互換，所以正確）；加了狀態之後必須是
「同 id **且狀態相同**才併」，否則兩條不同的魚會被併成一疊。
這條寫在 `ItemStack` 的類別註解裡。

**④ 空掉的疊要移除**，否則 `inventory` 會慢慢長滿 `count == 0` 的殘骸。

### `maxStack` 還沒做

今天多拿一把倉庫鑰匙只是浪費，不會壞任何東西。要加的時候：
欄位放 `ItemData.maxStack`，但**強制邏輯不要寫進 `RunContext`** ——
它是純資料、不該認識 `ItemDatabase`。包一層 `GameFlowManager.GrantItem(id, count)`，那裡才有資料庫。

### 已接上的顯示

重試詢問現在可以顯示剩餘數量：`{0}` 道具名、`{1}` 第幾次重試、`{2}` 還剩幾個。

> ⚠️ **既有 prefab 的文案不會自動更新** —— 改 C# 的預設值不會動到已序列化的字串。
> `chest_RequiresCheck` 的 `Retry Prompt Format` 目前是客製過的兩個佔位符版本，
> 要顯示剩餘數量的話自己把 `{2}` 加進去。少傳參數才會爆，多傳會被忽略，所以不加也不會壞。

## 5. 技術債與測試腳手架

### 5.1 測試腳手架 — 正式配內容前要還原

⚠️ **這些看起來像 bug，其實是刻意的。不要順手「修好」。**

| 項目 | 現況 | 還原成 |
|---|---|---|
| `RoomContent_Village` 的 `chest_RequiresKey` / `chest_Document` | `weight: 0`（讓測試寶箱必定出現） | 補回 1，否則這兩者永遠不生成 |
| `MapGenerationSettings.Use Demo Route` | **✓**（5 個全 Event 的直線路線） | 戰鬥接上後取消勾選。見 [DemoRoute.md](DemoRoute.md) |
| `MapNodeUI.Scale Hover`（若為了 demo 開了） | `1.1` | tooltip 上線後調回 `1` |

### 5.2 技術債

| 項目 | 影響 |
|---|---|
| **背包 UI 不存在** | 玩家看不到自己身上有什麼 |
| `lootItems`（顯示文字）與 `grantedItemIds`（id）是兩份要手動同步的清單 | 有了 `ItemData` 之後可以合併，但要重配現有寶箱資料 |
| `MetaProgressData.AddLegacyItem` 會去重，`RunContext.AddItem` 不會 | 語意上說得通（遺產＝解鎖、背包＝物資），但行為不一致 |
| 兩處玩家可見的中文寫死在程式裡 | `InspectableInteractable.cs:46`、`RoomController.cs:148`。在地化時的唯一障礙 |
| repo 沒有 `.editorconfig` | TYN 是 UTF-8、Romtyui 有檔案是 cp950，兩邊都沒 BOM |
| `_Archive/` 仍在編譯範圍 | 舊型別佔用全域命名空間（所以 `using` 要寫在 `namespace` 內） |
| `ExplorationDeck.OnCardPlayed` 沒有被呼叫 | 出過與沒出的牌都走 `DiscardHand`，所以 `CardDataExplore.exhaust` 目前無效 |

---

## 6. 建議順序

1. **commit**（目前這批已驗收）
2. **跟 Romtyui 談一次，A／B／C 三件一起提**（§2.1）—— 不擋任何事，但越早越好
3. **Phase 4d 收尾驗證**（小）—— 記得先把 `chest_RequiresKey` 的權重補回 1
4. **Phase 6 對話系統**（大）—— **劇情最依賴這個**，五名角色全部卡在這裡。
   C18①、C18③、商店都會併進來
5. Phase 5 戰鬥接入（等 A）、Phase 7 神牌、事件效果系統（等 §2.2 的分類決定）
