...
public class RelicsInventory : MonoBehaviour
{
    ...
    [Tooltip("暫時使用 ScriptableObject 儲存遺物資料。取得正式遺物資料類別後，再替換成對方的 RelicData 類型。")]
    [SerializeField]
    ...
    public bool AddRelic(ScriptableObject relic)
    {
        ...
        {
            Debug.LogWarning(
                "[RelicsInventory] AddRelic 失敗，relic 是 null"
            );
        ...
        Debug.Log(
            $"[RelicsInventory] 獲得遺物：{relic.name}"
        );
    ...
    public bool RemoveRelic(ScriptableObject relic)
    {
        ...
        Debug.Log(
            $"[RelicsInventory] 移除遺物：{relic.name}"
        );
    ...
    public bool RemoveRelicAt(int index)
    {
        ...
        {
            Debug.LogWarning(
                $"[RelicsInventory] RemoveRelicAt index 超出範圍：{index}"
            );
